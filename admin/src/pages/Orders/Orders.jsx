...
const AdminOrders = ({ url }) => {
  ...
  const printInvoice = (orderId) => {
    ...
    doc.setFontSize(14);
    doc.setFont("courier", "bold");
    doc.text("TOTAL :", totalCostX + 8, totalCostStartY);
    doc.text(`LKR ${(order.items.reduce((acc, item) => acc + (item.price * item.amount), 0)).toFixed(2)}`, totalCostX + 35, totalCostStartY);
